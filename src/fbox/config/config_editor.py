...
Usage:
    from fbox.config.config_editor import edit_config
...
from .config_bootstrap import ensure_config_exists
...
def edit_config(config: AppConfig) -> int:
    ...
    editor_parts = shlex.split(
        resolve_editor_command(config), posix=(sys.platform != "win32")
    )
